...
def test_headlines_keep_the_standing_rules():
    for key, spec in regen_library.CONCEPTS.items():
        ...
        if spec.get("set") in ("b2b", "platform", "platform_ads",
                               "summit_campaign"):
            # the cited exception: a swipe/event set stat headline is allowed
            # digits ONLY with a cite into the approved claims source (each
            # set's own test file proves every cite resolves and clears the gate)
            if re.search(r"[\d%]", h):
