...
--
-- 2. A UNIQUE index on (gym_id, late_account_id, metric_date) so the daily pull is an
--    UPSERT, not an append. Without it a re-pull silently doubles the series and every
--    growth number computed off it is wrong. This is the rail that makes the pipeline
