...
BRAND_PALETTE = (
    ...
    "Style: clean, professional, typographic data visualization on a light cream "
    "canvas. Premium B2B agency quality. No illustrated scenes, no photorealistic "
    "art, no AI-generated imagery, no gimmicks."
)
...
_HOUSE_STYLE_LEAD = (
    ...
    "All visual elements are simple flat icons, geometric shapes, structured data "
    "layouts, and clean diagrams. Never illustrated scenes, never people or figures "
    "in action, never photorealistic imagery, never 3D, never metaphorical concept "
    "art. Every card must look like it was designed by a professional human designer "
    "at a premium B2B agency, not generated by AI."
)
...
ARCHETYPES = {
    "flow": (
        "Archetype FLOW: the body of the card is a clean vertical STEP DIAGRAM, "
        "a structured sequence of flat labeled boxes or nodes connected by thin flow "
        "arrows, running top to bottom, with the headline at the top. No illustrated "
        "scenes, no people, no cartoon elements. Each node is a simple shape with one "
        "UPPERCASE business label (e.g. LEAD, BOOKED, SHOWED, CLOSED). Illustration "
        "scale medium, 3 to 5 steps; label density normal; the single red accent is "
        "ONE arrow or ONE node."
    ),
    "split": (
        "Archetype SPLIT: a clean two-column or two-row contrast layout, left vs right "
        "or top vs bottom, with the headline at the top. One side shows the 'before' "
        "state (muted, low contrast), one side shows the 'after' state (brand palette, "
        "high contrast). Each side has a simple flat data element or labeled box; no "
        "illustrated scenes, no figures. The ONE red element marks the winning side only. "
        "Label density low, one or two UPPERCASE labels per side."
    ),
    "hero": (
        "Archetype HERO: ONE LARGE clean data element (a bold stat block, a two-column "
        "comparison table, or a single oversized metric with context) fills most of the "
        "card, with the headline at the top and MAXIMUM negative space. The boldest, "
        "simplest read on the feed. No illustrated scenes, no cartoon figures. Label "
        "density minimal, at most TWO small UPPERCASE labels; the single red accent is "
        "one element inside the hero data visual."
    ),
    "path": (
        "Archetype PATH: a clean STAGED PROGRESSION diagram, a horizontal or vertical "
        "sequence of flat labeled stops showing a business journey from start to finish; "
        "the journey begins at the BOTTOM or LEFT and ends at the TOP or RIGHT, with the "
        "headline at the top. No illustrated scenes, no winding illustrated paths. "
        "Each stop is a simple shape with one UPPERCASE business label. Illustration "
        "scale medium; label density normal; the single red accent marks the FINAL stop."
    ),
    "headline": (
    ...
    "editorial": (
        ...
        "and COLOR alone. No illustration, no diagram, no figures, no vector scenes, "
        "no decorative icons. "
        "Structure (three levels, all left-aligned and left-anchored): "
        "1. EYEBROW: the small ALL CAPS letterspaced label at the very top left, "
        "rendered at that position per the concept spec. "
        "2. HEADLINE: large, bold, high-contrast editorial serif, left-aligned with "
        "tight leading. This is the hero moment — set it BIG. Give it the scale of a "
        "magazine cover, not a book title page. "
        "3. DECK: one medium-weight sans-serif line directly below the headline, "
        "rendered on the image. "
        "EXCEPTION: for this archetype, the eyebrow label, deck line, and any color "
        "block or visual anchor specified in the concept context ARE rendered exactly "
        "as described. "
        "Composition: EVERY ZONE EARNS ITS PLACE — color, type, or photo. No vacant "
        "thirds, no large empty fields. The concept context specifies the visual "
        "anchor (color block, photo treatment, or typographic scale); follow it exactly. "
        "Red used exactly once per the concept spec. "
        "Generous spacing between elements is allowed; an EMPTY top third is not."
    ),
}
...
STORY_REQUIREMENT = (
    "Quality standard (every card, every archetype): this card must look clean, "
    "professional, and high-end. The standard is premium B2B agency quality, "
    "the level of a top-tier SaaS brand. "
    "ALLOWED visual elements: simple flat icons (one or two maximum), structured "
    "tables or comparison layouts, minimal flow arrows, data callouts with numbers, "
    "before/after columns, clean labeled diagrams with meaningful business words. "
    "BANNED, do not generate any of these: illustrated scenes, people or figures "
    "in action, cartoon elements, metaphorical imagery (dripping money, safes, "
    "broken pipes, locks), photorealistic photography, 3D effects, gimmicky concept "
    "art, cluttered collages, generic abstract process labels (STEP 1, STEP 2, STEP 3, "
    "PLAN, GROW, LEARN, DISCOVER, LAUNCH, START, FINISH). "
    "Labels must use real business language a gym owner recognizes: LEADS, BOOKED, "
    "SHOWED, CLOSED, FOLLOW UP, AD SPEND, SIGNUPS, COST PER LEAD. "
    "The two-second test: a gym owner scrolling fast must read the card's point from "
    "the headline and the visual in under two seconds. Clean, direct, no decoding."
)
...
FEED_LAYOUT = (
    ...
    "the top or bottom edge. Put the entire headline BELOW the top safe band; keep the "
    "infographic and any footer word (e.g. a platform label) fully ABOVE the bottom safe "
    "band. Pull the key graphic toward the vertical CENTER so it also survives a square "
    "center-crop. Generous margins on all four sides. The complete headline and the "
    "complete key graphic must read in full with breathing room, nothing clipped."
)
