...
def _status():
    ...
    print(f"  gbp_publish    : {config.gbp_publish_enabled()}  (env AGENT_GBP_PUBLISH; Google Business publish + §7.2 reconcile lane; OFF => no GBP send; layered under AGENT_PUBLISH_ENABLED — draft unless BOTH armed)")
    print(f"  gbp_offer_ok   : {sorted(config.gbp_offer_confirmed_gyms()) or '(none)'}  (env AGENT_GBP_OFFER_CONFIRMED; GATE 1: gyms whose live offer is CONFIRMED; empty => OFFER posts OFF for all gyms)")
    print(f"  gbp_coach_scrn : {config.gbp_coach_screen_enabled()}  (env AGENT_GBP_COACH_SCREEN; GATE 2: first GBP month written 'coach_review' withheld from owner until released; default ON)")
    print(f"  coach_scrn_m1  : {config.coach_screen_first_month_enabled()}  (env AGENT_COACH_SCREEN_FIRST_MONTH; GATE 2 for the FB/IG client month: a gym's first month withheld 'coach_review' until a coach releases it; default ON; LASSO exempt)")
    print(f"  gbp_pub_window : {config.gbp_publish_window_enabled()}  (env AGENT_GBP_PUBLISH_WINDOW; §7.3/G5: gate GBP publishing to weekday 8-10am in the connection timezone; default ON; a row outside the window HOLDS)")
    print(f"  story_src_media: {config.story_source_media_enabled()}  (env AGENT_STORY_SOURCE_MEDIA; Task #28/§5c: store a story's raw source url so an edited story caption re-burns immediately; default OFF until the source_media_url migration is applied)")
    print(f"  vision_gyms    : {sorted(config.vision_gyms()) or '(none)'}  (env AGENT_VISION_GYMS; ECHO_VISION_SPEC: gyms with image understanding + grounded captions ON; empty => vision OFF for all; flips only at next month build)")
    print(f"  vision_allow_flags: {sorted(config.vision_allowed_flags()) or '(none)'}  (env AGENT_VISION_ALLOW_FLAGS; safety flags the planner may AUTO-PICK a photo despite — still detected/recorded, just no longer blocking; empty => every flag blocks (safe default); a business decision on brand/likeness/consent)")
    print(f"  client_video_edit: {config.client_video_edit_enabled()}  (env AGENT_CLIENT_VIDEO_EDIT; action-cut Reel editing for client videos — highest-motion fast cuts to ~{config.reel_target_sec():.0f}s (AGENT_REEL_TARGET_SEC), 9:16, hook text from the day's own approved caption; edits only ENHANCE, any failure posts the raw video; OFF => videos post as-is)")
    print(f"  story_format   : {config.story_format_enabled()}  (env AGENT_STORY_FORMAT; format a client PHOTO story into a filled 1080x1920 card — full photo on a blurred cover fill with the day's own approved caption burned in; ENHANCE only, any failure posts the raw photo; video stories untouched; OFF => raw photo posts)")
    ...
    print(f"  avatar_athlete_rail: {config.avatar_athlete_rail_enabled()}  (env AGENT_AVATAR_ATHLETE_RAIL; block captions addressing competitive-athlete audiences (hyrox, competitive crossfit, strength/serious athletes) at stage AND at the publish boundary. DEFAULT OFF since Blake's ruling 2026-09-01: 'confirmed we can talk about CrossFit, hyrox and competitive athletics' (first amended 2026-08-18). The machinery is intact — set this to true to re-arm the rail, and the per-gym hyrox allowlist (STORY_HYROX_AVATAR_GYMS) applies again. The workspace ORG INSTRUCTIONS still carry the older gen-pop-only wording; Blake owns reconciling that)")
    print(f"  billing_customer_sync: {config.billing_customer_sync_enabled()}  (env AGENT_BILLING_CUSTOMER_SYNC; copy the portal's gym_billing.stripe_customer_id onto Echo's own gyms row so publish_billing_gate can actually reach Stripe — the 2026-08-31 audit found the gate armed and structurally INERT because nothing had ever written that column. Reads the portal plane, writes ECHO's column only; never touches a client's Stripe account or subscription; never blanks a stored id; the gate keeps its fail-OPEN contract)")
    ...
    print(f"  welcome_per_day: {config.welcome_per_day()}  (env AGENT_WELCOME_PER_DAY)")
    ...
    print(f"  caption_cooldown: {config.caption_cooldown_enabled()}  (env AGENT_CAPTION_COOLDOWN; Wave 3 repeat-cooldown ledger + the HARD 180-day never-verbatim-twice rule at plan/stage/publish, default OFF)")
    print(f"  empty_cap_guard: {config.empty_caption_guard_enabled()}  (env AGENT_EMPTY_CAPTION_GUARD; stage-time belt: a feed row with zero visible caption characters never stages; publish-time belts are always on; default OFF)")
    print(f"  ask_coverage   : {config.ask_coverage_enabled()}  (env AGENT_ASK_COVERAGE; LASSO/B2B lane: every reel carries exactly one ask + month coverage floor {config.ask_coverage_floor()}% (AGENT_ASK_COVERAGE_FLOOR); default OFF)")
    ...
    print(f"  dedupe_content : {config.media_dedupe_by_content_enabled()}  (env AGENT_MEDIA_DEDUPE_BY_CONTENT; the repeat sweep refuses a replacement that is BYTE-IDENTICAL to a photo already on the book, default OFF)")
