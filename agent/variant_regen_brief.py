...
def generate_variant_from_brief(row, account_key, brief, client=None,
    ...
    if config.lasso_infographic_quality_enabled(account_key):
        ...
        try:
            copy = select_copy(str(row.get("caption") or brief))
        except ValueError:
    ...
    extra = {"cta": copy["cta"], "footer": copy.get("footer"), "art_direction": brief} if copy else {}
...
def _default_generate(headline, facts, client=None, aspect=None, pixels=None,
                      surface=None, account_key=None, cta="", footer=None, art_direction=""):
    from . import creative_studio
    return creative_studio.generate(
        headline, facts, client=client, aspect=aspect, pixels=pixels,
        surface=surface, account_key=account_key, cta=cta, footer=footer, art_direction=art_direction)
