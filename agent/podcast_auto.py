...
Headless note: motion b-roll renders via fal.ai REST API when AGENT_FAL_API_KEY +
AGENT_VIDEO_RENDER are both set. Without the key, overlays are skipped and reels
still get: animated intro card, host + word-highlight captions, Treatment B side
panels, and Nano Banana still cards (when AGENT_VIDEO_STILLS_ENABLED +
AGENT_NANO_API_KEY are set). Nothing publishes; every clip is a held PENDING draft
awaiting approval.
...
def run(source=None, render=False, account_key=None, client=None, poster=None,
    ...
    from . import higgsfield_renderer as _hf, fal_renderer as _fal
    _rndr = _hf.build_renderer() or _fal.build_renderer()
