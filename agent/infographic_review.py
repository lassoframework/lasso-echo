...
def evaluate(image_bytes, *, headline, facts, cta="", footer="", surface=None, vision_client=None):
    ...
    placement = ("Story: ALL essential text including headline, supporting facts, CTA and URL must lie entirely inside normalized x=0.06 to 0.94 and y=0.17 to 0.80. Text above or below this safe region is a MAJOR failure even if readable on the raw image. Estimate each text block against image dimensions and report which block crosses the boundary. Also inspect the full frame: a feed-size poster centered inside a 9:16 canvas, with broad empty top and bottom bands, is a MAJOR visual failure even when the file measures 1080x1920. The background and visual composition should fill the Story canvas while text remains in the safe region. "
