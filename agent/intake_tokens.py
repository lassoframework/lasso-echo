...
def mint(client_key, secret=None, db_conn=None):
...
# ---- onboard_verify compatibility shim ----------------------------------------
def token_status(account_key: str, db_conn=None) -> dict:
    """Compatibility shim for onboard_verify.  With HMAC signing, every gym's
    token is minted on demand from the shared secret — there is no per-gym row
    to look up.  Return ACTIVE when the signing secret is configured (any gym
    can get a signed link), NOT_SET when the secret is absent (operator hasn't
    configured the service yet)."""
    status = "ACTIVE" if _secret() is not None else "NOT_SET"
    return {"account_key": account_key, "status": status, "rotated_at": None}
