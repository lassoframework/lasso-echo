...
def _cited(bundle, category, domain):
    """[(fact, citation)] for one category, CITATION PRESERVED. The bible used to
    render `[f for f, _ in ...]`, throwing the source URL away the moment a fact
    left client_sources — so a reader (human or LLM) could not tell which page a
    line came from, and an unapproved scraped figure looked exactly like an
    approved one. Every fact this doc renders now names the page it was read from
    (falling back to the gym's own domain when a row carried no citation)."""
    out = []
    for item in bundle.get(category, []) or []:
        if isinstance(item, (tuple, list)) and len(item) >= 2:
            fact, cite = item[0], item[1]
        else:
            fact, cite = item, ""
        fact = str(fact or "").strip()
        if fact:
            out.append((fact, str(cite or "").strip() or str(domain or "")))
    return out
...
def _bible_text(gym_name, domain, bundle):
    ...
    STAMPED AUTO-DRAFTED. The first line carries voice.AUTO_DRAFTED_MARKER, so
    voice.load_voice returns auto_drafted=True and drafter._output_claims_cleared
    refuses to clear a caption's figures against this doc. A scrape is material
    for a human to review, never an approved source (finding 2026-09-06). Every
    fact keeps the URL it was read from."""
    from .bible_drafter import BASELINE_GUARDRAILS
    from .voice import AUTO_DRAFTED_MARKER
    about = _cited(bundle, "about", domain)
    services = _cited(bundle, "service", domain)
    pillars = [c for c in ("service", "educational", "offer", "faq", "about")
               if bundle.get(c)]
    # Rendered OUTSIDE the template: a multi-line expression inside an f-string
    # placeholder is a syntax error before Python 3.12, and CI runs 3.11.
    about_block = ("\n".join(f"{f} (source: {c})" for f, c in about)
                   if about else f"{gym_name}. See {domain}.")
    services_block = ("\n".join(f"- {s} (source: {c})" for s, c in services)
                      if services else f"- see {domain}")
    pillars_block = ("\n".join("- " + p for p in pillars) if pillars
                     else "- service")
    return f"""{AUTO_DRAFTED_MARKER}
# {gym_name} Brand Bible — Echo Social Agent (auto-drafted from {domain})

> AUTO-DRAFTED BY MACHINE, APPROVED BY NOBODY. Generated by website auto-intake
> from the gym's OWN website. Every fact below was read off {domain} and carries
> the page it came from; nothing is invented. Because no human has approved it,
> Echo does NOT treat this doc as an approved source of figures: a number here
> cannot clear the anti-fabrication gate. A human replacing or reviewing this doc
> (drop the marker on line 1) restores it to approved status. The POST approval
...
def _write_bible_if_missing(base, gym_name, domain, bundle):
    ...
    WHAT IT WRITES IS AN *AUTO-DRAFTED* BIBLE (see _bible_text): stamped
    voice.AUTO_DRAFTED_MARKER, so it can never clear a caption's figures through
    drafter._output_claims_cleared. CALLERS: this website auto-intake lane only.
    gym_deep_brain deliberately does NOT call it — a pre-onboarding scrape of a
    gym that is not yet a client never authors that gym's voice at all."""
