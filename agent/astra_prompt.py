...
def story_text_grid(pixels=None) -> str:
    ...
    return (
        ...
        "Nothing essential may extend below that destination band. On a corrective "
        "edit, any reviewer suggestion that names a lower divider or destination "
        "coordinate is diagnostic context only: ignore that coordinate and rebuild "
        "the complete lower text group inside these buffered divider and destination "
        "bands instead. These are bounding-box limits, not text anchors or prescribed "
        "visual rows. For this "
        ...
        "central visual before allowing any required text to leave its reserved band. "
        "Wrap and reflow copy to preserve every supplied word at a readable phone size. "
