#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym getbuffa68312.

WHY THIS EXISTS
----------------
Support ticket 52ab92a8-55a8-478c-bcaa-78f8830799ab, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake getbuffa68312: no domain on record
   (pass --domain or add the gym to portal_domains)"

getbuffa68312 is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for getbuffa68312
     using that domain, landing sources as PENDING for human review.
  3. A permanent entry for getbuffa68312 in agent/portal_domains.py (see the
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for getbuffa68312 using
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket 52ab92a8).
    "getbuffa68312": {
...
-----
    ...
    python3 scripts/intake_getbuffa68312.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_getbuffa68312.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_getbuffa68312.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_getbuffa68312.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_getbuffa68312.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_getbuffa68312 (dry_run={dry_run}, force={force})")
    print()
