#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym x4mountainbrook6bfc84.

WHY THIS EXISTS
----------------
Support ticket 10b4f914-f9e6-4f67-a590-bc76aad2501c, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake x4mountainbrook6bfc84: no domain on record
   (pass --domain or add the gym to portal_domains)"

x4mountainbrook6bfc84 is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for x4mountainbrook6bfc84
     using that domain, landing sources as PENDING for human review.
  3. A permanent entry for x4mountainbrook6bfc84 in agent/portal_domains.py (see the
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for x4mountainbrook6bfc84 using
  the operator-supplied --domain argument.
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket 10b4f914).
    "x4mountainbrook6bfc84": {
...
-----
    ...
    python3 scripts/intake_x4mountainbrook6bfc84.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_x4mountainbrook6bfc84.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_x4mountainbrook6bfc84.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_x4mountainbrook6bfc84.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_x4mountainbrook6bfc84.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_x4mountainbrook6bfc84 (dry_run={dry_run}, force={force})")
    print()
