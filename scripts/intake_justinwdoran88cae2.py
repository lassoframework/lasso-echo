#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym justinwdoran88cae2.

WHY THIS EXISTS
----------------
Support ticket 4ec58877-b9b9-4739-b2a9-a2345857c062, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake justinwdoran88cae2: no domain on record
   (pass --domain or add the gym to portal_domains)"

justinwdoran88cae2 is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for
     justinwdoran88cae2 using that domain, landing sources as PENDING for
     human review.
  3. A permanent entry for justinwdoran88cae2 in agent/portal_domains.py (see
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for justinwdoran88cae2
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket 4ec58877).
    "justinwdoran88cae2": {
...
-----
    ...
    python3 scripts/intake_justinwdoran88cae2.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_justinwdoran88cae2.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_justinwdoran88cae2.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_justinwdoran88cae2.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_justinwdoran88cae2.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_justinwdoran88cae2 (dry_run={dry_run}, force={force})")
    print()
