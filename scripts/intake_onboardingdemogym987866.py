#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym onboardingdemogym987866.

WHY THIS EXISTS
----------------
Support ticket c692d735-f7b2-4000-8172-1d4557cac637, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake onboardingdemogym987866: no domain on record
   (pass --domain or add the gym to portal_domains)"

onboardingdemogym987866 is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for
     onboardingdemogym987866 using that domain, landing sources as PENDING for
     human review.
  3. A permanent entry for onboardingdemogym987866 in agent/portal_domains.py (see
     the "AFTER RUNNING" section below) lets future auto-intake runs resolve this
     gym without needing --domain again.

NOTE: if onboardingdemogym987866 is a test or demo gym (not a real client), the
correct resolution is to NOT run this script and instead remove its echo_gym_settings
row from the database so the fleet sweep no longer treats it as an Echo client.
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for onboardingdemogym987866
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket c692d735).
    "onboardingdemogym987866": {
...
-----
    ...
    python3 scripts/intake_onboardingdemogym987866.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_onboardingdemogym987866.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_onboardingdemogym987866.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_onboardingdemogym987866.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_onboardingdemogym987866.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing.\n"
            f"\n"
            f"  NOTE: if this is a test/demo gym, do not run this script.\n"
            f"  Instead, remove its echo_gym_settings row from the database."
        )
        sys.exit(1)

    print(f"intake_onboardingdemogym987866 (dry_run={dry_run}, force={force})")
    print()
