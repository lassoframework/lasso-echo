#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym bryanjonesd046cd.

WHY THIS EXISTS
----------------
Support ticket 9e70ef96-e6d8-4c6a-85b1-8d5948083a13, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake bryanjonesd046cd: no domain on record
   (pass --domain or add the gym to portal_domains)"

bryanjonesd046cd is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for bryanjonesd046cd
     using that domain, landing sources as PENDING for human review.
  3. A permanent entry for bryanjonesd046cd in agent/portal_domains.py (see the
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for bryanjonesd046cd using
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket 9e70ef96).
    "bryanjonesd046cd": {
...
-----
    ...
    python3 scripts/intake_bryanjonesd046cd.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_bryanjonesd046cd.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_bryanjonesd046cd.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_bryanjonesd046cd.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_bryanjonesd046cd.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_bryanjonesd046cd (dry_run={dry_run}, force={force})")
    print()
