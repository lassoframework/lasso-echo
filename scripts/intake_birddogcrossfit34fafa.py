#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym birddogcrossfit34fafa.

WHY THIS EXISTS
----------------
Support ticket 71318fb6-2564-4f03-877d-2aba34d34cd9, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake birddogcrossfit34fafa: no domain on record
   (pass --domain or add the gym to portal_domains)"

birddogcrossfit34fafa is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for
     birddogcrossfit34fafa using that domain, landing sources as PENDING for
     human review.
  3. A permanent entry for birddogcrossfit34fafa in agent/portal_domains.py
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for birddogcrossfit34fafa
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket 71318fb6).
    "birddogcrossfit34fafa": {
...
-----
    ...
    python3 scripts/intake_birddogcrossfit34fafa.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_birddogcrossfit34fafa.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_birddogcrossfit34fafa.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_birddogcrossfit34fafa.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_birddogcrossfit34fafa.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_birddogcrossfit34fafa (dry_run={dry_run}, force={force})")
    print()
