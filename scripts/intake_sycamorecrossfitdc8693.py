#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym sycamorecrossfitdc8693.

WHY THIS EXISTS
----------------
Support ticket 993f6cc4-79be-40b2-be3e-848a12482131, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake sycamorecrossfitdc8693: no domain on record
   (pass --domain or add the gym to portal_domains)"

sycamorecrossfitdc8693 is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for
     sycamorecrossfitdc8693 using that domain, landing sources as PENDING for
     human review.
  3. A permanent entry for sycamorecrossfitdc8693 in agent/portal_domains.py
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for sycamorecrossfitdc8693
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket 993f6cc4).
    "sycamorecrossfitdc8693": {
...
-----
    ...
    python3 scripts/intake_sycamorecrossfitdc8693.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_sycamorecrossfitdc8693.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_sycamorecrossfitdc8693.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_sycamorecrossfitdc8693.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_sycamorecrossfitdc8693.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_sycamorecrossfitdc8693 (dry_run={dry_run}, force={force})")
    print()
