#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym nexusb5f055.

WHY THIS EXISTS
----------------
Support ticket a57408e6-2d8d-4e30-870d-46b3a80dd115, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake nexusb5f055: no domain on record
   (pass --domain or add the gym to portal_domains)"

nexusb5f055 is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for nexusb5f055
     using that domain, landing sources as PENDING for human review.
  3. A permanent entry for nexusb5f055 in agent/portal_domains.py (see the
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for nexusb5f055 using
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket a57408e6).
    "nexusb5f055": {
...
-----
    ...
    python3 scripts/intake_nexusb5f055.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_nexusb5f055.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_nexusb5f055.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_nexusb5f055.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_nexusb5f055.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_nexusb5f055 (dry_run={dry_run}, force={force})")
    print()
