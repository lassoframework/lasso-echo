#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym allkinecommunityandfitne35dc4d.

WHY THIS EXISTS
----------------
Support ticket 4dc3ec7f-e3a7-4199-8ddb-ee8852721d59, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake allkinecommunityandfitne35dc4d: no domain on record
   (pass --domain or add the gym to portal_domains)"

allkinecommunityandfitne35dc4d is not present in agent/portal_domains._REGISTRY, so
the auto-intake fleet sweep has no domain to scrape and skips the gym.
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for
     allkinecommunityandfitne35dc4d using that domain, landing sources as PENDING
     for human review.
  3. A permanent entry for allkinecommunityandfitne35dc4d in agent/portal_domains.py
     (see the "AFTER RUNNING" section below) lets future auto-intake runs resolve
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for
  allkinecommunityandfitne35dc4d using the operator-supplied --domain argument.
...
need --domain.  Add TWO entries: one keyed by the gym's human display name and
one keyed by the base key, as is the established pattern for this registry:

    # Looked up + verified YYYY-MM-DD (ticket 4dc3ec7f).
    "<Human Gym Name>": {
        "domain": "<the-real-domain-you-verified>",
    },
    "allkinecommunityandfitne35dc4d": {"domain": "<the-real-domain-you-verified>"},
...
-----
    ...
    python3 scripts/intake_allkinecommunityandfitne35dc4d.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_allkinecommunityandfitne35dc4d.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_allkinecommunityandfitne35dc4d.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_allkinecommunityandfitne35dc4d.py \\
...
_BASE = "allkinecommunityandfitne35dc4d"
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_allkinecommunityandfitne35dc4d.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_allkinecommunityandfitne35dc4d (dry_run={dry_run}, force={force})")
    print()
