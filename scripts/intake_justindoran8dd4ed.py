#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym justindoran8dd4ed.

WHY THIS EXISTS
----------------
Support ticket cdf07e2e-cb9e-48cd-ab8f-275ba6bc61c6, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake justindoran8dd4ed: no domain on record
   (pass --domain or add the gym to portal_domains)"

justindoran8dd4ed is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for justindoran8dd4ed
     using that domain, landing sources as PENDING for human review.
  3. A permanent entry for justindoran8dd4ed in agent/portal_domains.py (see the
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for justindoran8dd4ed using
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket cdf07e2e).
    "justindoran8dd4ed": {
...
-----
    ...
    python3 scripts/intake_justindoran8dd4ed.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_justindoran8dd4ed.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_justindoran8dd4ed.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_justindoran8dd4ed.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_justindoran8dd4ed.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_justindoran8dd4ed (dry_run={dry_run}, force={force})")
    print()
