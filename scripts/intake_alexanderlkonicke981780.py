#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym alexanderlkonicke981780.

WHY THIS EXISTS
----------------
Support ticket aba63ac0-3e78-40a2-8afc-f2ac70cc4d45, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake alexanderlkonicke981780: no domain on record
   (pass --domain or add the gym to portal_domains)"

alexanderlkonicke981780 is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for
     alexanderlkonicke981780 using that domain, landing sources as PENDING for
     human review.
  3. A permanent entry for alexanderlkonicke981780 in agent/portal_domains.py
     (see the "AFTER RUNNING" section below) lets future auto-intake runs
     resolve this gym without needing --domain again.
...
- Calls agent.website_intake.intake_from_website ONCE for
  alexanderlkonicke981780 using the operator-supplied --domain argument.
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket aba63ac0).
    "alexanderlkonicke981780": {
...
-----
    ...
    python3 scripts/intake_alexanderlkonicke981780.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_alexanderlkonicke981780.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_alexanderlkonicke981780.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_alexanderlkonicke981780.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_alexanderlkonicke981780.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_alexanderlkonicke981780 (dry_run={dry_run}, force={force})")
    print()
