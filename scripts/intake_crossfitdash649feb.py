#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym crossfitdash649feb.

WHY THIS EXISTS
----------------
Support ticket 3ed2bd1c-3879-4f6e-97c7-53c07f7f7c54, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake crossfitdash649feb: no domain on record
   (pass --domain or add the gym to portal_domains)"

crossfitdash649feb is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for
     crossfitdash649feb using that domain, landing sources as PENDING for
     human review.
  3. A permanent entry for crossfitdash649feb in agent/portal_domains.py (see
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for crossfitdash649feb
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket 3ed2bd1c).
    "crossfitdash649feb": {
...
-----
    ...
    python3 scripts/intake_crossfitdash649feb.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_crossfitdash649feb.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_crossfitdash649feb.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_crossfitdash649feb.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_crossfitdash649feb.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_crossfitdash649feb (dry_run={dry_run}, force={force})")
    print()
