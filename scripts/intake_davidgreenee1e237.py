#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym davidgreenee1e237.

WHY THIS EXISTS
----------------
Support ticket 900a0bc9-fcb6-48d2-9815-06d4a3d48415, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake davidgreenee1e237: no domain on record
   (pass --domain or add the gym to portal_domains)"

davidgreenee1e237 is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for
     davidgreenee1e237 using that domain, landing sources as PENDING for human
     review.
  3. A permanent entry for davidgreenee1e237 in agent/portal_domains.py (see
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for davidgreenee1e237
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket 900a0bc9).
    "davidgreenee1e237": {
...
-----
    ...
    python3 scripts/intake_davidgreenee1e237.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_davidgreenee1e237.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_davidgreenee1e237.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_davidgreenee1e237.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_davidgreenee1e237.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_davidgreenee1e237 (dry_run={dry_run}, force={force})")
    print()
