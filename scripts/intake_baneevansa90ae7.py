#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym baneevansa90ae7.

WHY THIS EXISTS
----------------
Support ticket 630d5069-0fc5-4e4c-8db9-793ded887d9f, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake baneevansa90ae7: no domain on record
   (pass --domain or add the gym to portal_domains)"

baneevansa90ae7 is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for
     baneevansa90ae7 using that domain, landing sources as PENDING for human
     review.
  3. A permanent entry for baneevansa90ae7 in agent/portal_domains.py (see
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for baneevansa90ae7
...
not need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket 630d5069).
    "baneevansa90ae7": {
...
-----
    ...
    python3 scripts/intake_baneevansa90ae7.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_baneevansa90ae7.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_baneevansa90ae7.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_baneevansa90ae7.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_baneevansa90ae7.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_baneevansa90ae7 (dry_run={dry_run}, force={force})")
    print()
