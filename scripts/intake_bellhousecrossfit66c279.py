#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym bellhousecrossfit66c279.

WHY THIS EXISTS
----------------
Support ticket f97bb8b7-155a-4efb-a99f-c6ccac5ef992, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake bellhousecrossfit66c279: no domain on record
   (pass --domain or add the gym to portal_domains)"

bellhousecrossfit66c279 is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for
     bellhousecrossfit66c279 using that domain, landing sources as PENDING for
     human review.
  3. A permanent entry for bellhousecrossfit66c279 in agent/portal_domains.py
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for bellhousecrossfit66c279
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket f97bb8b7).
    "bellhousecrossfit66c279": {
...
-----
    ...
    python3 scripts/intake_bellhousecrossfit66c279.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_bellhousecrossfit66c279.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_bellhousecrossfit66c279.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_bellhousecrossfit66c279.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_bellhousecrossfit66c279.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_bellhousecrossfit66c279 (dry_run={dry_run}, force={force})")
    print()
