#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym chrisfollickd955b4.

WHY THIS EXISTS
----------------
Support ticket 0b57e96d-4bb4-4a20-ba37-58c2b47a58bb, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake chrisfollickd955b4: no domain on record
   (pass --domain or add the gym to portal_domains)"

chrisfollickd955b4 is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for
     chrisfollickd955b4 using that domain, landing sources as PENDING for human
     review.
  3. A permanent entry for chrisfollickd955b4 in agent/portal_domains.py (see
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for chrisfollickd955b4
...
not need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket 0b57e96d).
    "chrisfollickd955b4": {
...
-----
    ...
    python3 scripts/intake_chrisfollickd955b4.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_chrisfollickd955b4.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_chrisfollickd955b4.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_chrisfollickd955b4.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_chrisfollickd955b4.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_chrisfollickd955b4 (dry_run={dry_run}, force={force})")
    print()
