#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym crossfitbesomeone9427a0.

WHY THIS EXISTS
----------------
Support ticket 62839378-93ec-4394-83f7-05d384c2ebb8, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake crossfitbesomeone9427a0: no domain on record
   (pass --domain or add the gym to portal_domains)"

crossfitbesomeone9427a0 is not present in agent/portal_domains._REGISTRY, so the
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for
     crossfitbesomeone9427a0 using that domain, landing sources as PENDING for
     human review.
  3. A permanent entry for crossfitbesomeone9427a0 in agent/portal_domains.py
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for crossfitbesomeone9427a0
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket 62839378).
    "crossfitbesomeone9427a0": {
...
-----
    ...
    python3 scripts/intake_crossfitbesomeone9427a0.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_crossfitbesomeone9427a0.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_crossfitbesomeone9427a0.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_crossfitbesomeone9427a0.py \\
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_crossfitbesomeone9427a0.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_crossfitbesomeone9427a0 (dry_run={dry_run}, force={force})")
    print()
