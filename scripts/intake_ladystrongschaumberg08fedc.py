#!/usr/bin/env python3
"""One-off ops script: website auto-intake for gym ladystrongschaumberg08fedc.

WHY THIS EXISTS
----------------
Support ticket 768a9ea8-3e42-47b4-916d-8afe8dd1ff6f, 2026-09-11. Echo raised:
  "ECHO ALERT: could not auto-intake ladystrongschaumberg08fedc: no domain on record
   (pass --domain or add the gym to portal_domains)"

ladystrongschaumberg08fedc is not present in agent/portal_domains._REGISTRY, so
...
No catalog action covers this case; the resolution is:
  ...
  2. This script runs website_intake.intake_from_website once for
     ladystrongschaumberg08fedc using that domain, landing sources as PENDING
     for human review.
  3. A permanent entry for ladystrongschaumberg08fedc in agent/portal_domains.py
...
----------------------
- Calls agent.website_intake.intake_from_website ONCE for ladystrongschaumberg08fedc
...
need --domain:
    ...
    # Looked up + verified YYYY-MM-DD (ticket 768a9ea8).
    "ladystrongschaumberg08fedc": {
...
-----
    ...
    python3 scripts/intake_ladystrongschaumberg08fedc.py --domain <gym-website.com> --dry-run

    # Live run -- lands sources as PENDING:
    python3 scripts/intake_ladystrongschaumberg08fedc.py --domain <gym-website.com>

    # Re-intake if sources already exist:
    python3 scripts/intake_ladystrongschaumberg08fedc.py --domain <gym-website.com> --force

Run from the repo root on the deployed Railway instance (shared volume + creds):
    railway ssh --service echo -- python3 scripts/intake_ladystrongschaumberg08fedc.py \\
...
_BASE = "ladystrongschaumberg08fedc"
...
def main():
    ...
    if not domain:
        print(
            ...
            f"    python3 scripts/intake_ladystrongschaumberg08fedc.py --domain <gym-website.com>\n"
            f"  Use --dry-run to preview without writing."
        )
        sys.exit(1)

    print(f"intake_ladystrongschaumberg08fedc (dry_run={dry_run}, force={force})")
    print()
