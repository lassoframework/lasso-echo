...
| AGENT_MEDIA_DEDUPE_BY_CONTENT | false | BLAKE | The repeat sweep refuses a replacement that is BYTE-IDENTICAL (size+sha256) to a photo already on the book, so it cannot "fix" a repeat with a copy of the very photo it is replacing. OFF is the pre-existing name-only check. The trade cuts both ways: when the ONLY spare is an identical copy, OFF swaps it in and reports the date fixed (a false fix); ON leaves the date and reports a small library (true, but the repeat stands). Arm by hand. |
