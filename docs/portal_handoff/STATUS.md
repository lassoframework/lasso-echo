...
| `GET /portal/<token>/social?month=YYYY-MM` | BUILT-behind-flag | `AGENT_PORTAL_SOCIAL_ENABLED` (default OFF => 404). Stripe social product ACTIVE else 402+empty. Month calendar for THIS gym: `{account_key,month,active,posts:[{day_key,status,pillar,format,image_public_url,caption}],recreate_budget:{limit,used,remaining},low_creative,days_remaining,awaiting_media,upload_url,media_review,fallback_episode,notice_state,upload_action}`. The media fields are read-only backend signals; `notice_state.status=unresolved` is not delivery confirmation. The separate frontend has not been verified to render them. Token isolation: every row scoped to account_key. |
